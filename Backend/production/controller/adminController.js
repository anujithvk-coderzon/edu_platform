...
const safeDeleteCourse = async (courseId, reason) => {
    try {
        ...
        if (course.thumbnail) {
            try {
                await (0, CDN_management_1.Delete_File)(course.thumbnail);
...
const RegisterUser = async (req, res) => {
    try {
        ...
        const emailResult = await (0, EmailVerification_1.StaffWelcomeEmail)(email, firstName, userRole);
...
const CheckTutorEmail = async (req, res) => {
    ...
    const existingAdmin = await DB_Config_1.default.admin.findUnique({ where: { email } });
    if (existingAdmin) {
        return res.status(400).json({
            success: false,
...
const SendTutorVerificationEmail = async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({
            success: false,
    ...
    const existingRequest = await DB_Config_1.default.tutorRequest.findUnique({ where: { email } });
    if (existingRequest) {
        return res.status(400).json({
            success: false,
    ...
    const emailResult = await (0, EmailVerification_1.sendTutorVerificationEmail)(email, otp);
    if (!emailResult.success) {
        return res.status(500).json({
            success: false,
...
const RegisterTutorPublic = async (req, res) => {
    try {
        ...
        const existingRequest = await DB_Config_1.default.tutorRequest.findUnique({
            where: { email }
        });
        if (existingRequest) {
            return res.status(400).json({
        ...
        const tutorRequest = await DB_Config_1.default.tutorRequest.create({
            data: {
                email,
                password: hashedPassword,
                firstName,
        ...
        const emailResult = await (0, EmailVerification_1.sendTutorPendingApprovalEmail)(email, firstName);
        if (!emailResult.success) {
...
const GetPendingTutorRequestsCount = async (req, res) => {
    try {
        ...
        const count = await DB_Config_1.default.tutorRequest.count();
        return res.json({
            success: true,
            data: { count }
        });
...
const GetAllTutorRequests = async (req, res) => {
    try {
        ...
        const requests = await DB_Config_1.default.tutorRequest.findMany({
            orderBy: { createdAt: 'desc' },
            select: {
                id: true,
                email: true,
...
const AcceptTutorRequest = async (req, res) => {
    try {
        ...
        const tutorRequest = await DB_Config_1.default.tutorRequest.findUnique({
            where: { id: requestId }
        });
        if (!tutorRequest) {
            return res.status(404).json({
        ...
        const tutor = await DB_Config_1.default.$transaction(async (tx) => {
        ...
        const emailResult = await (0, EmailVerification_1.sendTutorWelcomeEmail)(tutor.email, tutor.firstName);
        if (!emailResult.success) {
            console.error('Failed to send welcome email:', emailResult.error);
...
const RejectTutorRequest = async (req, res) => {
    try {
        ...
        const emailResult = await (0, EmailVerification_1.sendTutorRejectionEmail)(tutorRequest.email, tutorRequest.firstName);
        if (!emailResult.success) {
            console.error('Failed to send rejection email:', emailResult.error);
            // Don't fail the request rejection if email fails
        }
        // Delete the tutor request
        await DB_Config_1.default.tutorRequest.delete({
            where: { id: requestId }
        });
        return res.json({
            success: true,
...
const ToggleTutorStatus = async (req, res) => {
    try {
        ...
        // Update tutor status
        const updatedTutor = await DB_Config_1.default.admin.update({
            where: { id },
            data: { isActive },
            select: {
                id: true,
...
const CreateCourse = async (req, res) => {
    try {
        ...
        const course = await DB_Config_1.default.$transaction(async (tx) => {
...
const PublishCourse = async (req, res) => {
    try {
        ...
        await (0, EmailVerification_1.sendCoursePublishedEmail)(recipientEmail, recipientName, updatedCourse.title);
        return res.json({
            success: true,
            data: {
                course: updatedCourse,
...
const RejectCourse = async (req, res) => {
    try {
        ...
        const recipientName = updatedCourse.tutor ?
            `${updatedCourse.tutor.firstName} ${updatedCourse.tutor.lastName}` :
            `${updatedCourse.creator.firstName} ${updatedCourse.creator.lastName}`;
        await (0, EmailVerification_1.sendCourseRejectionEmail)(recipientEmail, recipientName, updatedCourse.title, reason || 'No reason provided');
        return res.json({
            success: true,
            data: {
                course: updatedCourse,
...
const GetPendingCoursesCount = async (req, res) => {
    try {
        ...
        const count = await DB_Config_1.default.course.count({
    ...
    try {
        ...
        for (const assignment of course.assignments) {
            ...
            for (const submission of submissionFiles) {
                if (submission.fileUrl) {
                    try {
                        // fileUrl should already be in format "folder/filename"
                        const deleted = await (0, CDN_management_1.Delete_File)(submission.fileUrl);
                        if (deleted) {
        ...
        if (course.thumbnail) {
            try {
                ...
                const thumbnailDeleted = await (0, CDN_management_1.Delete_File)(course.thumbnail);
                if (thumbnailDeleted) {
...
const CompleteMaterial = async (req, res) => {
    try {
        ...
        const stats = await (0, progressCalculator_1.recalculateAndUpdateProgress)(userId, material.courseId);
        return res.json({
            success: true,
            data: {
...
const GetEnrollmentProgress = async (req, res) => {
    try {
        ...
        const progressStats = await (0, progressCalculator_1.calculateCourseProgress)(userId, courseId);
        return res.json({
            success: true,
            data: {
                enrollment,
    ...
    try {
        ...
        if (courseId) {
            ...
            if (currentCourse?.thumbnail) {
                await (0, CDN_management_1.Delete_File)(currentCourse.thumbnail);
            }
...
const GetStudentsCount = async (req, res) => {
    try {
        const studentsCount = await DB_Config_1.default.student.count();
        return res.status(200).json({
            success: true,
            data: {
                studentsCount
...
const GetAllStudents = async (req, res) => {
    try {
        ...
        if (userRole === 'Admin') {
            ...
            const tutorCourses = await DB_Config_1.default.course.findMany({
                where: {
                    OR: [
        ...
        const students = await Promise.all(Array.from(studentMap.values()).map(async (student) => {
            ...
            for (const enrollment of student.enrollments) {
                ...
                const submittedAssignmentsData = await DB_Config_1.default.assignmentSubmission.findMany({
                    where: {
                        studentId: student.id,
                        assignment: {
                            courseId: enrollment.courseId
            ...
            const gradedAssignments = await DB_Config_1.default.assignmentSubmission.count({
                where: {
                    studentId: student.id,
                    assignment: {
                        courseId: { in: student.enrollments.map((e) => e.courseId) }
...
const UploadAdminAvatar = async (req, res) => {
    try {
        ...
        const currentAdmin = await DB_Config_1.default.admin.findUnique({
            where: { id: userId },
            select: { avatar: true }
        });
        // Upload new avatar to CDN
        console.log('☁️ Uploading new avatar to CDN');
        const avatarUrl = await (0, CDN_management_1.Upload_Files)('avatars', req.file);
        if (!avatarUrl) {
            return res.status(500).json({
                success: false,
        ...
        if (currentAdmin?.avatar) {
            try {
                ...
                const deleted = await (0, CDN_management_1.Delete_File)(currentAdmin.avatar);
                if (deleted) {
...
const CleanupOrphanedCourses = async (req, res) => {
    try {
        ...
        const potentialOrphans = await DB_Config_1.default.course.findMany({
            where: {
                AND: [
                    { status: client_1.CourseStatus.DRAFT },
