      ...
      try {
        ...
        if (userResponse.success) {
          ...
          if (coursesResponse.success) {
            ...
            // Goals can be fetched from user settings or calculated dynamically
            const courseGoal = Math.max(10, courses.length * 2); // Dynamic goal based on current progress
            const studentGoal = Math.max(100, totalStudents * 3); // Scale with current student count
            const earningsGoal = Math.max(1000, totalEarnings * 2); // Scale with current earnings
            const viewsGoal = Math.max(500, totalStudents * 2); // Scale with student base
            
            // Calculate monthly views based on course engagement
            // This would ideally come from actual analytics data
            const avgViewsPerStudent = courses.length > 0 ? 2 : 0; // More realistic estimate
