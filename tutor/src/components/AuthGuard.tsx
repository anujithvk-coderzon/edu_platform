...
export function AuthGuard({ children }: AuthGuardProps) {
  ...
  useEffect(() => {
    ...
    // Check if user has the right role (ADMIN only)
  ...
  // Don't render if user doesn't have the right role (unless on public route)
  if (!publicRoutes.includes(pathname) && user && user.role !== 'ADMIN') {
    return null; // Will redirect in useEffect
  }
